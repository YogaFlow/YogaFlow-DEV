/*
  Ensure users promoted from participant role no longer appear in course participants.

  Changes:
  1) On role change user -> teacher/admin/owner:
     - delete only future registrations for that user in same tenant.
  2) Harden register_for_course:
     - only role 'user' may register.
*/
...
DROP TRIGGER IF EXISTS cleanup_future_registrations_on_role_upgrade ON public.users;
CREATE TRIGGER cleanup_future_registrations_on_role_upgrade
  AFTER UPDATE OF role ON public.users
  FOR EACH ROW
  EXECUTE FUNCTION public.cleanup_future_registrations_on_role_upgrade();
