...
- **Lokale Entwicklung (`localhost`):** Es wird **keine** Produktions-Subdomain in den Link geschrieben, auch wenn `APP_BASE_DOMAIN` gesetzt ist. Stattdessen `http://localhost:5173/verify-email?token=…&tenant={slug}` (Parameter `tenant` setzt den DEV-Studio-Kontext nach `signOut`).
