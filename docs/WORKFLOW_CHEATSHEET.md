...
- **Release nur über Pull Request** – Änderungen von `Julius` nach `main` mit mindestens einer Review; dann baut und deployed **Cloudflare** automatisch (Git + Wrangler).
...
5. Merge in `main` → Cloudflare baut und deployed; Live-Seite nutzt **PROD**-Datenbank (Build-Variablen in Cloudflare).
...
| Archiv: Migration Bolt → Netlify (historisch) | [BOLT_ZU_NETLIFY_WORKFLOW.md](BOLT_ZU_NETLIFY_WORKFLOW.md) |
| Asana-Checkliste Feature bis Live | [ASANA_CHECKLISTE_FEATURE_LIVE.md](ASANA_CHECKLISTE_FEATURE_LIVE.md) |

## PROD-Keys

- Nur in **Cloudflare** (Build-Variablen für Production) und ggf. im Passwortmanager ablegen.
