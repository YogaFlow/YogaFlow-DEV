      ...
      <div className="min-w-0 flex-1">
        <div className="flex justify-between items-start gap-2 mb-0.5">
          ...
          <div className="flex flex-col items-end shrink-0">
            <p
              className={`text-xs shrink-0 ${
                conversation.unreadCount > 0
                  ? `${isActive ? 'text-brandOnSoft' : 'text-brand'} font-medium`
                  : 'text-textMuted'
              }`}
            >
            ...
            {conversation.unreadCount > 0 && (
              <span className="inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1 mt-1 text-xs font-semibold text-brandOnSoft tabular-nums rounded-full border border-brandSoft bg-brandSoft">
