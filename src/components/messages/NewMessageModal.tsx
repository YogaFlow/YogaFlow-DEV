...
const NewMessageModal: React.FC<NewMessageModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`w-full sm:max-w-lg max-h-[95vh] overflow-y-auto rounded-t-lg sm:rounded-lg border border-border bg-surface shadow-lg transition-all duration-200 ${
      ...
      >
        <div className="sticky top-0 bg-surface border-b border-border px-6 py-4 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-sm hover:bg-surfaceSunken text-textMuted transition-colors"
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 border border-border rounded-sm focus:ring-2 focus:ring-brand focus:border-transparent text-base"
              required
            >
            ...
            <div>
              <label className="flex items-center gap-2 text-sm font-medium text-textMuted">
                <input
                  ...
                  className="rounded-sm border-border text-brand focus:ring-brand"
                />
            ...
            <div className="relative">
              ...
              <div className="absolute bottom-2 right-2" ref={pickerRef}>
                <button
                  ...
                  className="p-1.5 rounded-sm text-textSubtle hover:text-brandPressed hover:bg-surfaceSunken"
                  aria-label="Emoji auswählen"
                >
                  <Smile size={18} />
                </button>
                {emojiOpen && (
                  <div className="absolute bottom-full right-0 mb-2 z-50 shadow-lg rounded-md overflow-hidden">
                    <EmojiPicker
          ...
          <button
            type="submit"
            disabled={sending}
            className="w-full flex items-center justify-center gap-2 bg-brand text-onBrand px-4 py-2.5 rounded-sm hover:bg-brandPressed transition-colors disabled:opacity-50 text-sm font-medium"
          >
            <Send size={18} />
            Nachricht senden
          </button>
